...
The isolated UI/UX demo release `7f54e84` reached Ready in Vercel Production. A stable-route check confirmed that `/pm-ui-demo` renders the separate Project Manager `Delivery pulse.` concept and its persistent non-live boundary without opening a PM production workspace.

The paired `/hr-ui-demo` route also renders the separate Human Resources `Workforce clarity.` concept, its wide-screen-only guard, and its non-live boundary. It presents only static capacity, leave, staffing, and people-focus examples; it contains no payroll action, API request, or live personnel record.
