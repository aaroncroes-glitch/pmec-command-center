...
describe("PMEC shared delivery contract", () => {
  it("stores assignment identity, employee routing, and delivery progress", () => {
    expect(Object.keys(getTableColumns(pmecAssignments))).toEqual(
  ...
  it("stores employee time logs with assignment routing and review state", () => {
    expect(Object.keys(getTableColumns(pmecTimeLogs))).toEqual(
      expect.arrayContaining([
        "id",
        "assignmentId",
        "employeeId",
  ...
  it("stores employee notification routing, read state, and archive state", () => {
    expect(Object.keys(getTableColumns(pmecNotifications))).toEqual(
      expect.arrayContaining([
        "id",
        "employeeId",
        "type",
  ...
  it("stores employee-specific notification preference controls", () => {
    expect(Object.keys(getTableColumns(pmecNotificationPreferences))).toEqual(
      expect.arrayContaining([
        "employeeId",
        "assignmentsEnabled",
        "timeApprovedEnabled",
  ...
  it("keeps server-side assignment writes PM-only while HR receives only organization time-log read access", () => {
    expect(deliveryRouter).toContain(
  ...
  it("shows access boundaries in the employee and control-center dashboards while using a server-side Clerk principal", () => {
    ...
    const controlCenter = readFileSync(
      resolve(process.cwd(), "app/control-center/index.tsx"),
      "utf8",
    );
    ...
    expect(controlCenter).toContain("WHO CAN DO WHAT");
    expect(controlCenter).toContain("What your account can do is highlighted below.");
  });
});
