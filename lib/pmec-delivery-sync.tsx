...
export function PmecDeliverySyncProvider({ children }: PropsWithChildren) {
  ...
  const notifications = trpc.pmecDelivery.notifications.useQuery({ employeeId }, { refetchInterval: 12_000 });
  ...
  const markNotificationMutation = trpc.pmecDelivery.markNotificationRead.useMutation();
  const refresh = useCallback(async () => { await Promise.all([utils.pmecDelivery.assignments.invalidate(), utils.pmecDelivery.timeLogs.invalidate(), utils.pmecDelivery.notifications.invalidate()]); }, [utils]);
  ...
  const sharedNotifications = (notifications.data ?? []) as SharedNotification[];
  const value = useMemo<SyncContext>(() => ({ ready: !allAssignments.isLoading && !allTimeLogs.isLoading && !notifications.isLoading, employeeId, employeeName, employeeAssignments: (employeeAssignments.data ?? []) as SharedAssignment[], allAssignments: (allAssignments.data ?? []) as SharedAssignment[], employeeTimeLogs: (employeeTimeLogs.data ?? []) as SharedTimeLog[], allTimeLogs: (allTimeLogs.data ?? []) as SharedTimeLog[], notifications: sharedNotifications, unreadNotifications: sharedNotifications.filter((item) => !item.readAt).length, assign, updateAssignment, submitTime, reviewTime, markNotificationRead, refresh }), [allAssignments.data, allAssignments.isLoading, allTimeLogs.data, allTimeLogs.isLoading, assign, employeeAssignments.data, employeeId, employeeName, employeeTimeLogs.data, markNotificationRead, notifications.isLoading, refresh, reviewTime, sharedNotifications, submitTime, updateAssignment]);
