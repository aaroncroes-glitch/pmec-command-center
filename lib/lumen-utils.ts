...
export function workingDayCount(startDate: string, endDate: string, holidayDates: string[] = []) { const start = new Date(`${startDate}T12:00:00`); const end = new Date(`${endDate}T12:00:00`); if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime()) || end < start) return 0; const holidays = new Set(holidayDates); let total = 0; for (const day = new Date(start); day <= end; day.setDate(day.getDate() + 1)) { const key = toDateKey(day); if (day.getDay() !== 0 && day.getDay() !== 6 && !holidays.has(key)) total += 1; } return total; }
